...
function getRandomDate() {
  ...
  const daysLater = Math.floor(Math.random() * 60) + 7;
...
async function main() {
  ...
  const userIds = await new Promise((resolve) => {
    db.all('SELECT id FROM user LIMIT 10', (err, users) => {
      resolve(users.map(u => u.id));
    });
  });
  console.log(`   找到 ${userIds.length} 个用户\n`);
  
  console.log('📦 开始为每个分类创建活动...\n');
  ...
  for (const categoryId of Object.keys(eventTemplates)) {
    ...
    console.log(`      已有 ${existingCount} 个活动，需要创建 ${needToCreate} 个`);
    
    for (let i = 0; i < needToCreate; i++) {
      const title = getRandomItem(template.titles);
      const date = getRandomDate();
      const organizerId = getRandomItem(userIds) || 1;
      ...
      await new Promise((resolve) => {
        db.run(`
          INSERT INTO event (
            ...
            status, organizerId, createdAt, viewCount
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), ?)
        `, [
          `${title} #${i + 1}`,
